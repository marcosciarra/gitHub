(function (angular) {
    'use strict';
    angular.module('angularjs-gauge', []).directive('ngGauge', gaugeMeterDirective).provider('ngGauge', gaugeMeterProviderFn);
    ...
    function gaugeMeterDirective(ngGauge) {
        ...
        Gauge.prototype = {
            init: function () {
                ...
                var lfs = this.options.size * 0.22, llh = this.options.size;
                this.text.css({
                ...
                this.unit.css({textDecoration: 'none', fontSize: '0.6em', fontWeight: 200, opacity: 0.8});
                ...
                var self = this, type = this.getType(), bounds = this.getBounds(type), duration = this.getDuration(),
                    min = this.getMin(), max = this.getMax(), value = this.clamp(this.getValue(), min, max),
                    head = bounds.head, unit = (bounds.tail - bounds.head) / (max - min),
                    displacement = unit * (value - min), tail = bounds.tail,
                    color = this.getForegroundColorByRange(value), requestID, starttime;
                ...
                var
                    context = this.context, center = this.getCenter(), radius = this.getRadius(),
                    foregroundColor = color, backgroundColor = this.getBackgroundColor();
                this.clear();
                context.beginPath();
                context.strokeStyle = backgroundColor;
                context.arc(center.x, center.y, radius, middle, tail, !1);
                context.stroke();
                context.beginPath();
                context.strokeStyle = foregroundColor;
                context.arc(center.x, center.y, radius, start, middle, !1);
                context.stroke()
            }, clear: function () {
                ...
                var x = this.getWidth() / 2, y = this.getHeight() / 2;
                return {x: x, y: y}
            }, getValue: function () {
                ...
                var match = Object.keys(this.options.thresholds).filter(function (item) {
        ...
        return {
            ...
            link: function (scope, element) {
                var defaults = ngGauge.getOptions();
                ...
                scope.$watch('value', watchData, !1);
                scope.$watch('min', watchData, !1);
                scope.$watch('max', watchData, !1);
                scope.$watch('cap', watchOther, !1);
                scope.$watch('thick', watchOther, !1);
                scope.$watch('type', watchOther, !1);
                scope.$watch('size', watchOther, !1);
                scope.$watch('duration', watchOther, !1);
                scope.$watch('foregroundColor', watchOther, !1);
                scope.$watch('backgroundColor', watchOther, !1);
                scope.$watch('thresholds', watchData, !1);
